...
uint8_t _workMode  = MOTION_DETECTION_MODE;
...
status_t LIS3DH::begin( uint8_t pin1 , uint8_t pin2 , intFuncPtr intHandle1 , intFuncPtr intHandle2)
{
	//Begin the inherited core.  This gets the physical wires connected
	status_t returnError = beginCore();
	...
	pinMode(pin1, INPUT);
	pinMode(pin2, INPUT);
  attachInterrupt(pin1, intHandle1, RISING);
	attachInterrupt(pin2, intHandle2, RISING);
...
//
//****************************************************************************//
//#define DEFAULT_ODR 	0x27 // ODR = 10 Hz, LPM disable, xyz enable
#define DEFAULT_ODR   0x2F // ODR = 10 Hz, LPM enable, xyz enable

void LIS3DH::setMode( workMode_t mode )
{
	uint8_t dataRead;
	if( mode == MOTION_DETECTION_MODE ) // Normal mode 10HZ 10 bit  4 uA 16mg wake up.
	{
		_workMode = MOTION_DETECTION_MODE;
		writeRegister(LIS3DH_CTRL_REG1, 0x1F); // ODR = 10 Hz Turn on the sensor, enable X, Y, and Z.
		// writeRegister(LIS3DH_CTRL_REG2, 0x09); // High-pass filter enabled on data and interrupt1.
		// writeRegister(LIS3DH_CTRL_REG3, 0x40); // Interrupt driven to INT1 pad.
		// writeRegister(LIS3DH_CTRL_REG4, 0x00); // FS = 2 g.
		// writeRegister(LIS3DH_CTRL_REG5, 0x08); // Interrupt latched.
		// writeRegister(LIS3DH_INT1_THS, 0x02);  // Threshold = 32mg.
		// writeRegister(LIS3DH_INT1_DURATION, 0x00);  // Duration = 0.

		// writeRegister(LIS3DH_INT1_CFG, 0x2A);  // Configure desired wake-up event.
		
		writeRegister(LIS3DH_CTRL_REG2, 0x0B); // High-pass filter enabled on data and interrupt1.
		writeRegister(LIS3DH_CTRL_REG3, 0x40); // Interrupt driven to INT1 pad.
		writeRegister(LIS3DH_CTRL_REG6, 0x20); // Interrupt driven to INT2 pad.
		writeRegister(LIS3DH_CTRL_REG4, 0x00); // FS = 2 g.
		writeRegister(LIS3DH_CTRL_REG5, 0x0A); // Interrupt latched.
		writeRegister(LIS3DH_INT1_THS, 0x0F);  // Threshold = 250mg.
		writeRegister(LIS3DH_INT2_THS, 0x05);  // Threshold = 80mg.
		writeRegister(LIS3DH_INT1_DURATION, 0x00);  // Duration = 0.
		writeRegister(LIS3DH_INT2_DURATION, 0x00);  // Duration = 0.

		writeRegister(LIS3DH_INT1_CFG, 0x6A);  // Configure desired wake-up event.
		writeRegister(LIS3DH_INT2_CFG, 0x6A);  // Configure desired wake-up event.
	}
	else if( mode == SHOCK_DETECTION_MODE )
	{
		_workMode = SHOCK_DETECTION_MODE;
		writeRegister(LIS3DH_CTRL_REG1, DEFAULT_ODR); // ODR = 10 Hz Turn on the sensor, enable X, Y, and Z.
		// writeRegister(LIS3DH_CTRL_REG2, 0x09); // High-pass filter enabled on data and interrupt1.
		// writeRegister(LIS3DH_CTRL_REG3, 0x40); // Interrupt driven to INT1 pad.
		// writeRegister(LIS3DH_CTRL_REG4, 0x20); // FS = 8 g.
		// writeRegister(LIS3DH_CTRL_REG5, 0x08); // Interrupt latched.
		// writeRegister(LIS3DH_INT1_THS, 0x1F);  // Threshold = 2000mg.
		// writeRegister(LIS3DH_INT1_DURATION, 0x00);  // Duration = 0.
		...
		writeRegister(LIS3DH_CTRL_REG4, 0x20); // FS = 8 g.
		writeRegister(LIS3DH_CTRL_REG5, 0x0A); // Interrupt latched.
		writeRegister(LIS3DH_INT1_THS, 0x5F);  // Threshold = 6000mg.
		writeRegister(LIS3DH_INT2_THS, 0x1F);  // Threshold = 2000mg.
		writeRegister(LIS3DH_INT1_DURATION, 0x00);  // Duration = 0.
		writeRegister(LIS3DH_INT2_DURATION, 0x00);  // Duration = 0.

		writeRegister(LIS3DH_INT1_CFG, 0x2A);  // Configure desired wake-up event.
		writeRegister(LIS3DH_INT2_CFG, 0x2A);  // Configure desired wake-up event.
	}
	readRegister(&dataRead, LIS3DH_INT1_SRC);//cleared by reading
	readRegister(&dataRead, LIS3DH_INT2_SRC);//cleared by reading
}
...
//
//****************************************************************************//
#define COLLECTIONCOUNT   100
eventType_t LIS3DH::getEventType( uint32_t  timeOut )
{
	
	float xDate[COLLECTIONCOUNT];
	float yDate[COLLECTIONCOUNT];
	float zDate[COLLECTIONCOUNT];
	
	writeRegister(LIS3DH_CTRL_REG1, 0x97); // 设置加速度输出速率为 1.344 kHz
	for(int i = 0; i < COLLECTIONCOUNT; i++)
	{
		xDate[i] = readFloatAccelX();
		yDate[i] = readFloatAccelY();
		zDate[i] = readFloatAccelZ();
		delay(1);
	}
	writeRegister(LIS3DH_CTRL_REG1, DEFAULT_ODR); // 设置加速度输出速率为 1.344 kHz
	
	if( _workMode == MOTION_DETECTION_MODE )
	{
		for(int i = 0; i < COLLECTIONCOUNT; i++)
		{
			if((xDate[i] > 0.08) || (yDate[i] > 0.08) || (zDate[i] > 0.08))
			{
				return MOTION_HIGH;
			}
			else if((xDate[i] > 0.05) || (yDate[i] > 0.05) || (zDate[i] > 0.05))
			{
				return MOTION_MEDIUM;
			}
			else if((xDate[i] > 0.02) || (yDate[i] > 0.02) || (zDate[i] > 0.02))
			{
				return MOTION_LOW;
			}
		}
	...
	else if( _workMode == SHOCK_DETECTION_MODE )
	{
		for(int i = 0; i < COLLECTIONCOUNT; i++)
		{
			if((xDate[i] > 6) || (yDate[i] > 6) || (zDate[i] > 6))
			{
				return SHOCK_HIGH;
			}
			else if((xDate[i] > 4) || (yDate[i] > 4) || (zDate[i] > 4))
			{
				return SHOCK_MEDIUM;
			}
			else if((xDate[i] > 2) || (yDate[i] > 2) || (zDate[i] > 2))
			{
				return SHOCK_LOW;
			}
		}
...
void LIS3DH::applySettings( void )
{
  ...
  dataToWrite |= 0x01 << 3; // low power mode
	//Now, write the patched together data
#ifdef VERBOSE_SERIAL
