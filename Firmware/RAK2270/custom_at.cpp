...
bool get_at_setting(uint32_t setting_type)
{
  ...
  case SEND_FREQ_OFFSET:
    ...
    {
      //Serial.printf("No valid send frequency found, set to default, read 0X%02X 0X%02X 0X%02X 0X%02X\r\n",
...
//    Serial.printf("Read send frequency 0X%02X 0X%02X 0X%02X 0X%02X",
//        flash_value[0], flash_value[1],
//        flash_value[2], flash_value[3]);
    g_txInterval = 0;
    ...
    //Serial.printf("Send frequency found %ld", g_txInterval);
...
bool save_at_setting(uint32_t setting_type)
{
  ...
  switch (setting_type)
  {
    case  SEND_FREQ_OFFSET:
          ...
          flash_value[4] = 0xAA; // 
//          Serial.printf("AT_CMD", "Writing send frequency 0X%02X 0X%02X 0X%02X 0X%02X ",
//              flash_value[0], flash_value[1],
//              flash_value[2], flash_value[3]);
...
void init_factory_mode()
{
  ...
  //Serial.printf("FACTORY_MODE: %s(%d)\r\n", g_FACTORY_MODE?"ON":"OFF", g_FACTORY_MODE);
  if (g_FACTORY_MODE != FACTORY_MODE_OFF && g_FACTORY_MODE != FACTORY_MODE_ON) {
    //Serial.print("Access flash data first time.\r\n");
    //g_FACTORY_MODE = FACTORY_MODE_ON;
  }
}
...
/**
 * @brief handle event mode
 */
uint8_t g_event_mode = DEFAULT_EVENT_MODE;

void init_event_mode()
{
  uint8_t event_mode = 0xFF;
  if (api.system.flash.get(EVENT_MODE_OFFSET, &event_mode, 1) == false) {
    Serial.print("Reading flash data failure...\r\n");
  }
  //Serial.printf("Read EVENT_MODE: %d\r\n", event_mode);
  if (event_mode == 0xFF)
    g_event_mode = DEFAULT_EVENT_MODE;
  else
...
{
  if (api.system.flash.set(EVENT_MODE_OFFSET, &event_mode, 1) == false) {
    Serial.print("Writing flash data failure...\r\n");
    return false;
  }
  g_event_mode = event_mode;
  return true;
